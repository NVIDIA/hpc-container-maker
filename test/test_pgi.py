...
r'''# PGI compiler version 18.04
RUN apt-get update -y && \
    apt-get install -y --no-install-recommends \
        libnuma1 \
        wget && \
    rm -rf /var/lib/apt/lists/*
RUN mkdir -p /tmp/pgi && wget -q --no-check-certificate -O /tmp/pgi/pgi-community-linux-x64-latest.tar.gz --referer https://www.pgroup.com/products/community.htm?utm_source=nvidia_hpccm\&utm_medium=wgt\&utm_campaign=CE\&nvid=nv-int-hccrmr-37128 -P /tmp/pgi https://www.pgroup.com/support/downloader.php?file=pgi-community-linux-x64 && \
...
ENV LD_LIBRARY_PATH=/opt/pgi/linux86-64/18.04/lib:$LD_LIBRARY_PATH \
    PATH=/opt/pgi/linux86-64/18.04/bin:$PATH''')
...
RUN apt-get update -y && \
    apt-get install -y --no-install-recommends \
        libnuma1 && \
    rm -rf /var/lib/apt/lists/*
COPY --from=0 /opt/pgi/linux86-64/18.04/REDIST/*.so /opt/pgi/linux86-64/18.04/lib/
RUN ln -s /opt/pgi/linux86-64/18.04/lib/libpgnuma.so /opt/pgi/linux86-64/18.04/lib/libnuma.so
ENV LD_LIBRARY_PATH=/opt/pgi/linux86-64/18.04/lib:$LD_LIBRARY_PATH''')
